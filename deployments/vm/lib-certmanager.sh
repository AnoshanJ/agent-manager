...
cert_dns_names() {
  ...
  # env-Thunder handles sit directly under the base domain (no "thunder." segment
  # — see thunder-naming.sh's thunder_host), so they're already covered by
  # AMP_HOST_THUNDER's own base-domain wildcard below and need no separate SAN
  # of their own — unlike the gateway tier, which still nests one level under
  # AMP_HOST_GATEWAY and needs its own explicit wildcard.
  printf '*.%s\n' "$AMP_AGENTS_BASE"
