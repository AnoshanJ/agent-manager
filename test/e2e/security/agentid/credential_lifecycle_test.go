...
var _ = Describe("SEC-AGENTID-001: external AgentID credential lifecycle", Label("security", "agentid"), Ordered, func() {
	...
	It("provisions the environment identity without exposing a secret in safe reads", func() {
		var completed framework.AgentIdentityEnvironmentView
		Eventually(func(g Gomega) string {
			views, _ := agentops.ListAgentIdentities(g, adminClient,
		...
		}).WithTimeout(cfg.ReadinessTimeout).WithPolling(2 * time.Second).Should(Equal("completed"))

		Expect(completed.AgentID).NotTo(BeEmpty())
		Expect(completed.ClientID).NotTo(BeEmpty())

		_, raw := agentops.ListAgentIdentities(Default, adminClient,
			cfg.DefaultOrg, cfg.DefaultProject, agentName, cfg.DefaultEnv)
		expectNoClientSecretField(raw, "completed AgentID safe-read response")
	})

	It("rejects an unauthenticated identity read with 401", func() {
		resp, err := adminClient.GetUnauthenticated(identityPath + "?" +
	...
	It("denies AgentID read, rotation, and revocation without amp:agent:update", func() {
		query := "?" + url.Values{"environment": {cfg.DefaultEnv}}.Encode()

		resp, err := reducedClient.Get(identityPath + query)
		Expect(err).NotTo(HaveOccurred())
		framework.ExpectForbidden(Default, resp, "under-scoped AgentID read")
		resp.Body.Close()

		resp, err = reducedClient.Post(identityPath,
		...
		resp, err = reducedClient.Delete(identityPath + query)
		Expect(err).NotTo(HaveOccurred())
		framework.ExpectForbidden(Default, resp, "under-scoped AgentID secret revocation")
		resp.Body.Close()
	})

	It("generates a credential that can mint an environment token", func() {
		generated := agentops.RegenerateAgentIdentitySecret(Default, adminClient,
			cfg.DefaultOrg, cfg.DefaultProject, agentName, cfg.DefaultEnv)
		...
		expectCredentialAccepted(clientID, currentSecret)
	})

	It("invalidates the old secret when the credential is rotated", func() {
		...
		expectCredentialRejected(clientID, firstSecret)
		expectCredentialAccepted(clientID, currentSecret)
	})

	It("revokes the current secret without echoing it and prevents new tokens", func() {
		...
		expectCredentialRejected(clientID, currentSecret)

		_, safeRead := agentops.ListAgentIdentities(Default, adminClient,
			cfg.DefaultOrg, cfg.DefaultProject, agentName, cfg.DefaultEnv)
...
func expectCredentialAccepted(clientID, clientSecret string) {
	Eventually(func(g Gomega) framework.ClientCredentialsTokenResult {
		result, err := framework.RequestClientCredentialsToken(cfg.AgentIDTokenURL, clientID, clientSecret)
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(result.StatusCode).To(Equal(http.StatusOK))
		g.Expect(result.AccessToken).NotTo(BeEmpty())
		return result
	}).WithTimeout(30 * time.Second).WithPolling(time.Second).Should(HaveField("StatusCode", http.StatusOK))
}

func expectCredentialRejected(clientID, clientSecret string) {
	Eventually(func(g Gomega) string {
		result, err := framework.RequestClientCredentialsToken(cfg.AgentIDTokenURL, clientID, clientSecret)
		g.Expect(err).NotTo(HaveOccurred())
		g.Expect(result.StatusCode).To(Or(Equal(http.StatusBadRequest), Equal(http.StatusUnauthorized)))
		return result.OAuthError
	}).WithTimeout(30 * time.Second).WithPolling(time.Second).Should(Equal("invalid_client"))
}
