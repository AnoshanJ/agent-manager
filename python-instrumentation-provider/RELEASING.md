...
| **Loose dependency inputs** (extras beyond `traceloop-sdk`) | `python-instrumentation-provider/requirements.in` |
| **Pinned dependency locks** (frozen full tree per `traceloop × python`) | `python-instrumentation-provider/locks/traceloop-<T>-python<X.Y>.txt` — generated by `make lock` (a **point-in-time** re-resolution of live PyPI; re-running it later can legitimately bump transitive pins). The image installs these directly, so builds are reproducible. CI does **not** re-resolve; the hermetic `make check-locks` (**Instrumentation Provider Lock Check**) only verifies every `release-config.json` entry has a matching lock with the right `traceloop-sdk` pin |
...
5. **Re-lock dependencies** so the new `(traceloop × python)` set is frozen (the
   image installs the lock, not a live resolution):
   ```bash
   cd python-instrumentation-provider && make lock
   git add locks/ && git commit -m "Re-lock instrumentation provider dependencies"
   ```
   Commit the new/updated `locks/` in the same PR. The **Instrumentation Provider
   Lock Check** CI job fails if you skip this.
6. **Regenerate the server's embedded catalog**. The `agent-manager-service` binary
...
7. **Publish the images** by dispatching the **`AMP Python Instrumentation Image Release`**
...
8. **Make it the platform default** (when you want *new* agents to get `0.4.0`).
...
3. **Re-lock dependencies** for the new `(traceloop × python)` combinations:
   ```bash
   cd python-instrumentation-provider && make lock
   git add locks/ && git commit -m "Re-lock instrumentation provider dependencies"
   ```
   Commit the new `locks/` files (one per added Python × each traceloop version) in
   the same PR; the **Instrumentation Provider Lock Check** CI job enforces this.
4. **Regenerate the server's embedded catalog** so the new Python flows into each
   entry's `pythonVersions` and the Console picks it up:
   ```bash
   cd agent-manager-service && make gen-instrumentation-baseline
   ```
5. **Publish the images** by dispatching the **`AMP Python Instrumentation Image Release`**
   workflow with the affected `instrumentation_version` (or `all`) to push the new
   `(instr × 3.14)` images.
6. **No PyPI change** — `amp-instrumentation` isn't Python-version-specific (the per-Python
...
7. **Docs**: update the supported-Python list / mapping table in `amp-instrumentation.mdx`.
