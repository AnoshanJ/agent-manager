...
// MonitorLLMProviderRef references an org-level LLM provider by handle.
// Used in create/update requests — the backend creates a gateway proxy for the provider.
type MonitorLLMProviderRef struct {
	ProviderName string `json:"providerName" validate:"required,min=1"` // handle of org-level LLM provider
}

// MonitorLLMProviderInfo describes the LLM provider configured for a monitor (API response).
type MonitorLLMProviderInfo struct {
	ProviderName   string `json:"providerName"`             // org-level provider handle
	DisplayName    string `json:"displayName"`              // human-readable name
	TemplateHandle string `json:"templateHandle,omitempty"` // e.g. "openai", "anthropic"
}
