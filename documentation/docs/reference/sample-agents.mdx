...
A hotel assistant that searches availability, creates and cancels bookings, and answers policy questions from hotel policy PDFs.

This is the most involved sample: it talks to external APIs and is split across two deployables, the agent plus a separate Hotel API service that its tools call over HTTP. Run the Hotel API first, somewhere the agent can reach. For a platform-hosted agent that means a deployed URL or a tunnel such as ngrok, not `localhost`. It can run platform-hosted or externally-hosted; the [sample README](https://github.com/wso2/agent-manager/tree/main/samples/hotel-booking-agent) covers both.

- **App Path** — `/samples/hotel-booking-agent/agent`, which is a subdirectory rather than the sample root
- **Agent requires** — `OPENAI_API_KEY` and `HOTEL_API_BASE_URL`
- **Hotel API requires** — `OPENAI_API_KEY`, plus `PINECONE_API_KEY` to store policies in Pinecone. Without a Pinecone key it uses an in-memory store that is rebuilt on every start.
